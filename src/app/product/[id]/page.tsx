...
const productData: Record<string, any> = {
  '1': {
    ...
    description:
      'ست لانژری رز گلد Hilda با پارچه ابریشم مصنیعی درجه یک و طراحی منحصربه‌فرد ساخته شده. این ست شامل سوتین و شورت با جزئیات ظریف تور و روبان است. مناسب برای هدیه دادن یا خودتان!',
    rating: 4.9,
    reviews: 128,
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    colors: [
    ...
    careInstructions: 'شستشو با دست یا ماشین لباسشویی با آب سرد. خشک کردن در سایه.',
    isNew: true,
  },
};
...
const productReviews = [
  { name: 'مریم', rating: 5, date: '۳ روز پیش', text: 'واقعاً عالیه! کیفیت پارچه فوق‌العاده‌ست و سایزش دقیقاً مثل جدول سایز بود.', avatar: '👩' },
  { name: 'سارا', rating: 5, date: '۱ هفته پیش', text: 'خیلی شیک و زیباست. بسته‌بندی هم خیلی خاص بود.', avatar: '👩‍🦰' },
  { name: 'نیلوفر', rating: 4, date: '۲ هفته پیش', text: 'محصول خوبیه فقط رنگش یکم با عکس فرق داشت ولی در کل راضیم.', avatar: '👩‍🦱' },
];
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-hilda-blush/10 mb-4">
              ...
              />
              {product.isNew && (
                <span className="absolute top-4 right-4 bg-hilda-wine text-white text-xs px-3 py-1.5 rounded-full font-medium z-10">
                  جدید
                </span>
              )}
                ...
                {product.images.map((img: string, i: number) => (
                  <button
                    ...
                    className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all duration-300 ${
                      activeImage === i ? 'border-hilda-wine' : 'border-transparent'
                    }`}
                  >
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            {/* Title & Rating */}
            <h1 className="font-display text-3xl md:text-4xl font-light text-hilda-charcoal mb-3">
              {product.name}
            </h1>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              {product.originalPrice && (
                <span className="text-lg text-hilda-charcoal/30 line-through">{product.originalPrice.toLocaleString('fa-IR')}</span>
              )}
            </div>
                ...
                {product.colors.map((color: any, i: number) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-full border-2 transition-all duration-300 ${
                      selectedColor === i ? 'border-hilda-wine scale-110 shadow-lg' : 'border-hilda-blush/30 hover:scale-105'
                    }`}
                ...
                {product.sizes.map((size: string) => (
                  <button
                    ...
                    className={`w-12 h-12 rounded-xl text-sm font-medium transition-all duration-300 ${
                      selectedSize === size
                        ? 'bg-hilda-burgundy text-white shadow-lg shadow-hilda-burgundy/30'
                        : 'bg-hilda-blush/10 text-hilda-charcoal hover:bg-hilda-blush/20'
                    }`}
                  >
                    {size}
                  </button>
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center bg-hilda-blush/10 rounded-xl">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-3 hover:bg-hilda-blush/20 rounded-r-xl transition-colors">
                    <Minus size={16} />
                  </button>
                  <span className="w-12 text-center font-medium">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="p-3 hover:bg-hilda-blush/20 rounded-l-xl transition-colors">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
            ...
            <div className="flex gap-3 mb-8">
              <button
                ...
                className={`flex-1 btn-luxury rounded-xl flex items-center justify-center gap-2 transition-all duration-300 ${
                  addedToCart
                    ? 'bg-green-500 text-white'
                    : 'bg-hilda-burgundy text-white hover:shadow-xl hover:shadow-hilda-burgundy/30'
                }`}
              >
                <ShoppingBag size={18} />
              ...
              <button
                ...
                className={`p-3.5 rounded-xl border-2 transition-all duration-300 ${
                  isWishlisted ? 'bg-hilda-wine text-white border-hilda-wine' : 'border-hilda-blush/30 hover:border-hilda-wine hover:text-hilda-wine'
                }`}
              >
                <Heart size={20} fill={isWishlisted ? 'currentColor' : 'none'} />
              </button>
              <button className="p-3.5 rounded-xl border-2 border-hilda-blush/30 hover:border-hilda-wine hover:text-hilda-wine transition-all duration-300">
                <Share2 size={20} />
              </button>
            </div>
            ...
            {/* Features */}
            <div className="border-t border-hilda-blush/20 pt-6 mb-6">
              <h3 className="text-sm font-medium text-hilda-charcoal mb-3">ویژگی‌ها:</h3>
              <ul className="space-y-2">
                {product.features.map((f: string, i: number) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-hilda-charcoal/60 font-light">
                    <span className="w-1.5 h-1.5 rounded-full bg-hilda-rose flex-shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
            ...
            <div className="border-t border-hilda-blush/20 pt-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="text-sm text-hilda-wine hover:underline flex items-center gap-1">
